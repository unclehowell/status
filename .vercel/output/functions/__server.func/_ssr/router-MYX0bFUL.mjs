...
//#region node_modules/.nitro/vite/services/ssr/assets/router-MYX0bFUL.js
...
var styles_default = "/assets/styles-nudagjX7.css";
var APP_NAME = "Enquiry";
var APP_DESC = "FOI · GDPR · SAR enquiry tree for Great House Farm, Llandough / BP Properties Ltd v Buckler 1987. Remaining avenues before legal action.";
var Route$6 = createRootRoute({
	...
	component: () => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("html", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("head", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HeadContent, {}) }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("body", {
			className: "min-h-dvh bg-bg font-mono text-fg",
			children: [
...
var $$splitComponentImporter = () => import("./routes-GRICJizB.mjs");
...
function advanced(statuses) {
	if (!statuses.length) return "open";
	const live = statuses.filter((s) => LIVE[s] != null);
	if (live.length) return live.reduce((a, b) => (LIVE[b] ?? 0) >= (LIVE[a] ?? 0) ? b : a);
	if (statuses.includes("open")) return "open";
	if (statuses.includes("bounce") && !statuses.includes("exhausted")) return "bounce";
	if (statuses.includes("bounce")) return "bounce";
	return "exhausted";
}
function inferStatus(emails) {
	if (!emails.length) return "open";
	const last = emails[emails.length - 1];
	const hasOut = emails.some((e) => e.direction === "out");
	const hasIn = emails.some((e) => e.direction === "in");
	const blob = emails.map((e) => `${e.subject} ${e.preview}`).join(" ").toLowerCase();
	if (blob.includes("address not found") || blob.includes("wasn't delivered")) return "bounce";
	if (blob.includes("internal review") || blob.includes("review of") || blob.includes("atisn") && blob.includes("review")) return "review";
	if (blob.includes("information commissioner") || blob.includes("ombudsman") || blob.includes("iopc")) return "complaint";
	if (hasOut && !hasIn) return "sent";
	if (hasIn && last.direction === "out") return "awaiting";
	if (hasIn) return "replied";
	return "sent";
}
function gather(overlays, ids) {
	const emails = [];
	const statuses = [];
	let reference;
	let note;
...
function node(id, label, kind, description, how, overlay, extra) {
...
function withChildren(id, label, kind, description, how, children, overlay, extra) {
	const emails = [...overlay.emails ?? []];
	for (const c of children) emails.push(...c.emails ?? []);
	emails.sort((a, b) => a.date.localeCompare(b.date));
	const status = advanced([overlay.status ?? (emails.length ? inferStatus(emails) : "open"), ...children.map((c) => c.status)]);
	return {
		id,
		label,
		kind,
...
function buildAvenues(_track, overlays, o = {}) {
	...
	const foiReview = gather(overlays, [
		"foi.review",
		"foi.ico",
		"foi.tribunal"
	]);
	const sarBody = gather(overlays, [
		"sar.submit",
		"sar.id",
		"sar.response",
		"sar"
	...
	const gdprBody = gather(overlays, [
		"gdpr.submit",
		"gdpr",
		"gdpr.response"
	]);
	const gdprReview = gather(overlays, [
		"gdpr.review",
		"gdpr.ico",
		"sar.ico"
	]);
	const enquiryBody = gather(overlays, EXTRA_ENQUIRY_KEYS);
	const foiReviewNode = node("foi.review", "internal review", "foi", "Ask the authority to review its FOI / EIR handling, search, and any exemption. Next: ICO, then the tribunal.", "Write 'I request an internal review' and list the defects. Typical window 20–40 working days. If the review is late or still withholds, complain to the ICO.", foiReview, {
		policyUrl: o.foiPolicy,
		contact: o.foiContact
	});
	const gdprReviewNode = node("gdpr.review", "internal review", "sar", "Challenge a GDPR / DPA response with the DPO, then the ICO data-protection complaint.", "Write to the DPO listing withheld classes. If still incomplete, ico.org.uk data-protection complaint (separate from an FOI complaint).", gdprReview.status || gdprReview.emails?.length ? gdprReview : {
		...gdprReview,
		status: "open"
	}, {
		policyUrl: o.sarPolicy,
		contact: o.sarContact
	});
	const sarReviewNode = node("sar.review", "internal review", "sar", "Challenge a subject-access response with the DPO before the ICO.", "List each withheld class and why the exemption is not engaged. Then ICO data-protection complaint.", sarReview, {
		policyUrl: o.sarPolicy,
		contact: o.sarContact
	});
	return [withChildren("enquiry", "enquiry", "enquiry", "Top-level enquiry with this party. Expand for foi, gdpr and sar. Colour follows the live correspondence.", "Open foi, gdpr or sar. Each expands to its internal review. Policy links and emails sit in the panel.", [
		withChildren("foi", "foi", "foi", "Freedom of Information / Environmental Information. Colour is the live FOI email status.", "Submit a record-lifecycle request. Diary the 20 working-day clock. Then expand for internal review.", [foiReviewNode], foiBody, {
			policyUrl: o.foiPolicy,
			contact: o.foiContact
		}),
		withChildren("gdpr", "gdpr", "sar", "UK GDPR / DPA 2018 — how they process personal data of named Williams/Buckler family members.", "A data-protection complaint is distinct from a SAR. Expand for the internal review / ICO step.", [gdprReviewNode], gdprBody.emails?.length || gdprBody.status ? gdprBody : {
			...gdprBody,
			status: "open"
		}, {
			policyUrl: o.sarPolicy,
		...
		withChildren("sar", "sar", "sar", "Subject access (UK GDPR Art. 15) for named living family members. Not a route to a deceased relative's file.", "Write to the DPO. One month. Expand for internal review.", [sarReviewNode], sarBody, {
			policyUrl: o.sarPolicy,
			contact: o.sarContact
		})
	], enquiryBody, { contact: o.foiContact ?? o.sarContact ?? o.complaintContact })];
}
function e(date, direction, subject, from, to, preview, reference) {
	return {
		date,
		direction,
		subject,
...
var ME = "Sion Buckler <hywelapbuckler@gmail.com>";
var PARTIES = [
	{
		id: "cadw",
		...
		policies: [
			{
				label: "Welsh Government FOI / EIR",
				url: "https://www.gov.wales/freedom-information"
			},
			{
				label: "Cadw contact / customer support",
				url: "https://cadw.gov.wales/about-us/contact-us"
			},
			{
				label: "Welsh Government complaints",
				url: "https://www.gov.wales/complain-about-welsh-government"
			},
			{
				label: "PSOW — how to complain",
				url: "https://www.ombudsman.wales/how-to-complain/"
			},
			{
				label: "ICO FOI complaints",
				url: "https://ico.org.uk/make-a-complaint/foi-and-eir-complaints/"
			}
		],
		notes: "ATISN 27021 submitted June 2026, internal review by Amy Longford, then Dr Andy Wigley on surviving records and historic retention. cadw@gov.wales and cadw@wales.gsi.gov.uk bounce. Use Andy.Wigley@gov.wales and the TfW customer-support channel. ICO and PSOW still remaining.",
		...
		overlays: {
			"foi.identify": {
				status: "exhausted",
				note: "Cadw is a Welsh Government service; FOI/EIR is the right regime. ATISN 27021 allocated."
			},
			"foi.submit": {
				status: "exhausted",
				reference: "ATISN 27021",
				emails: [e("2026-06-01", "out", "FOI / EIR — Great House Farm, Llandough (ATISN 27021)", ME, "Amy.Longford@gov.wales", "Initial FOI/EIR to Cadw concerning the 1988 spot-listing file and record lifecycle.", "ATISN 27021")]
			},
			"foi.ack": {
				status: "exhausted",
				reference: "ATISN 27021"
			...
			"foi.ico": {
				status: "open",
				note: "Next statutory step once the Wigley/Longford review correspondence is treated as closed or overdue."
			},
			"foi.tribunal": { status: "open" },
			"enquiry.informal": {
				status: "replied",
				emails: [e("2026-08-29", "in", "Re: Great House Farm, Llandough #611801922", "cadw@customersupport.tfw.wales", ME, "Customer-support channel acknowledged and passed to the relevant team.")]
			},
			"enquiry.retention": {
				status: "open",
				note: "Send a formal preservation notice addressed to Cadw and Welsh Government records management."
			},
			"complaint.s1": {
				status: "open",
				note: "FOI review is not a substitute for a maladministration complaint about the 1988 listing decision and the lost file."
			},
	...
	},
	{
		...
		policies: [
			{
				label: "RCAHMW FOI",
				url: "https://rcahmw.gov.uk/about-us/corporate-information/freedom-of-information/"
			},
			{
				label: "Coflein catalogue",
		...
		overlays: {
			"enquiry.informal": {
				status: "awaiting",
				emails: [e("2026-08-14", "in", "RE: CADW failures – Revised Report (10 August 2026)", "NMR Enquiries <nmr.wales@rcahmw.gov.uk>", ME, "Acknowledged the report; forwarded to Online Services to look into the matter.")]
			},
			"foi.submit": {
				status: "open",
				note: "Direct FOI still remaining: 1974 survey provenance, any 1988 contact with Cadw, Coflein omissions."
			}
		}
	},
	{
		...
		emails: [
			"enquiries@heneb.org.uk",
			"her@heneb.org.uk",
			"Barbara.Rees@heneb.org.uk"
		],
		policies: [{
			label: "Archwilio HER",
			url: "https://www.archwilio.org.uk/"
		}, {
			label: "Heneb",
			url: "https://heneb.org.uk/"
		}],
		...
		overlays: {
			...
			"foi.submit": {
				status: "exhausted",
				note: "Not an FOIA public authority."
			},
			...
			"foi.ico": {
				status: "exhausted",
				note: "ICO FOI complaint does not lie against a private limited company."
			},
			...
			"enquiry.catalogue": {
				status: "open",
				note: "Search Archwilio for GGAT 1978–80 / 1989–90 assessments around the demolition."
			},
			"enquiry.retention": {
				status: "sent",
				emails: [e("2026-05-29", "out", "FORMAL DISCLOSURE NOTICE & LITIGATION HOLD: Historic Significance, Planning Approvals, and Displaced Family Tenancy Records for Great House Farm, Llandough (FOIA/EIR Request)", ME, "Barbara.Rees@heneb.org.uk", "Litigation hold / disclosure notice.")]
			}
		}
	},
	{
		...
		policies: [{
			label: "Museum Wales FOI",
			url: "https://museum.wales/freedom-of-information/"
		}, {
			label: "ICO FOI complaints",
			url: "https://ico.org.uk/make-a-complaint/foi-and-eir-complaints/"
		}],
		...
		overlays: {
			"foi.submit": {
				...
				emails: [e("2026-07-27", "out", "FOI - 2026-024 - Great House Farm, Llandough", ME, "FOI.Requests@museumwales.ac.uk", "Request concerning records of a reported discovery at Great House Farm including a soldier.", "FOI 2026-024")]
			},
			"foi.response": {
				status: "exhausted",
				reference: "FOI 2026-024",
				emails: [e("2026-08-25", "in", "FOI - 2026-024 - Great House Farm, Llandough", "FOI Requests <FOI.Requests@museumwales.ac.uk>", ME, "Response to the 27 July request.")]
			},
			"foi.review": {
				status: "review",
				reference: "FOI 2026-024",
				emails: [e("2026-08-26", "out", "Re: FOI - 2026-024 - Great House Farm, Llandough", ME, "FOI.Requests@museumwales.ac.uk", "Internal review requested: searches appear to have been insufficient."), e("2026-08-26", "in", "RE: FOI - 2026-024 - Great House Farm, Llandough", "FOI.Requests@museumwales.ac.uk", ME, "Internal review accepted.")]
			}
		}
	},
	{
		...
		policies: [{
			label: "Glamorgan Archives",
			url: "https://glamarchives.gov.uk/"
		}, {
			label: "Cardiff Council FOI",
			url: "https://www.cardiff.gov.uk/ENG/Your-Council/Data-protection-and-FOI/Pages/default.aspx"
		}],
	...
	},
	{
		...
		policies: [
			{
				label: "TNA FOI",
				url: "https://www.nationalarchives.gov.uk/contact-us/freedom-of-information/"
			},
			{
				label: "Discovery catalogue",
		...
		overlays: {
			"foi.submit": {
				status: "exhausted",
				emails: [e("2026-09-02", "in", "RE: Freedom of Information request - Great House Farm, Llandough, Cardiff (BP vs Buckler 1987) CRM:007302446", "FOI Requests <FOIRequests@nationalarchives.gov.uk>", "request-1495300-08f92a1d@whatdotheyknow.com", "Clarification received; request taken off hold; respond by 23/09/2026.", "CRM:007302446")]
			},
			"foi.ack": {
				status: "exhausted",
				deadline: "2026-09-23"
			...
			"foi.response": {
				status: "awaiting",
				deadline: "2026-09-23",
				note: "Clock running. Diary 23 Sep 2026."
			}
		}
	},
	{
		...
		policies: [{
			label: "NLW FOI",
			url: "https://www.library.wales/about-nlw/governance/freedom-of-information"
		}, {
			label: "Welsh Newspapers Online",
	...
	},
	{
		...
		overlays: { "enquiry.informal": {
			status: "bounce",
			emails: [e("2026-08-23", "out", "Great House Farm (alias Cedwyn/ Cedfin)", ME, "archives@mountstuart.com", "Enquiry about historical Bute Estate records concerning Great House Farm / Ty Mawr."), e("2026-08-23", "in", "Delivery Status Notification (Failure)", "Mail Delivery Subsystem", ME, "Address not found: archives@mountstuart.com.")]
		} }
	},
	{
		id: "hmlr",
		...
		policies: [
			{
				label: "Get information about property",
				url: "https://www.gov.uk/get-information-about-property-and-land"
			},
			{
				label: "HMLR complaints procedure",
				url: "https://www.gov.uk/government/organisations/land-registry/about/complaints-procedure"
			},
			{
				label: "HMLR FOI",
				url: "https://www.gov.uk/government/organisations/land-registry/about/publication-scheme"
			},
			{
				label: "ICO",
	...
	},
	{
		...
		policies: [
			{
				label: "Vale FOI",
				url: "https://www.valeofglamorgan.gov.uk/en/our_council/Freedom-of-Information.aspx"
			},
			{
				label: "Complaints and compliments",
				url: "https://www.valeofglamorgan.gov.uk/en/our_council/Complaints-and-Compliments.aspx"
			},
			{
				label: "Members' code of conduct",
				url: "https://www.valeofglamorgan.gov.uk/en/our_council/Councillors-and-Committees/Councillors/Standards-Committee.aspx"
			},
			{
				label: "PSOW",
				url: "https://www.ombudsman.wales/how-to-complain/"
		...
		overlays: {
			...
			"foi.review": {
				...
				emails: [e("2026-08-30", "out", "Great House Farm, Llandough", ME, "FoiUnit@valeofglamorgan.gov.uk", "Chase of internal review of Case Ref 00210772, requested 7 July 2026.", "00210772"), e("2026-09-01", "out", "Great House Farm, Llandough – Request 00210772 – Internal Review – Formal Chase", ME, "FoiUnit@valeofglamorgan.gov.uk", "Further formal chase of the internal review.", "00210772")]
			},
			"foi.ico": {
				status: "open",
				note: "Review is overdue. ICO complaint is now available."
			},
			"complaint.s1": {
				status: "review",
				emails: [
	...
	},
	{
		...
		policies: [{
			label: "NRW access to information",
			url: "https://naturalresources.wales/about-us/what-we-do/how-we-work/access-to-information/?lang=en"
		}, {
			label: "PSOW",
			url: "https://www.ombudsman.wales/how-to-complain/"
		}],
		...
		overlays: { "enquiry.informal": {
			status: "sent",
			emails: [e("2026-08-21", "in", "Automatic reply: Llandough Project, Cardiff", "South East Planning <southeastplanning@cyfoethnaturiolcymru.gov.uk>", ME, "Auto-ack: dealing with the enquiry.")]
		} }
	},
	{
		id: "pedw",
		...
		policies: [{
			label: "PEDW",
			url: "https://www.gov.wales/planning-and-environment-decisions-wales"
		}],
		notes: "dns.wales@planninginspectorate.gov.uk is dead (PINS Wales closed). Use PEDW. Casework FOI/EIR remaining.",
		overlays: { "enquiry.informal": {
			status: "bounce",
			emails: [e("2026-08-21", "in", "Automatic reply: Llandough Project, Cardiff", "dns.wales@planninginspectorate.gov.uk", ME, "This address is no longer in use; Planning Inspectorate in Wales has transferred to PEDW.")]
		} }
	},
	{
		id: "companies-house",
		...
		policies: [{
			label: "Find and update company information",
			url: "https://find-and-update.company-information.service.gov.uk/"
		}, {
			label: "Complaints",
			url: "https://www.gov.uk/government/organisations/companies-house/about/complaints-procedure"
		}],
		notes: "Public search is free and should be done before any letter before claim. FOI is rarely needed.",
		overlays: { "enquiry.catalogue": {
			status: "open",
			note: "Pull filing histories for BP Properties, BP Pension Trust, WGR and any successor."
		} }
	},
	{
		id: "bp",
		...
		policies: [{
			label: "BP privacy / contact",
			url: "https://www.bp.com/en/global/corporate/privacy-statement.html"
		}, {
		...
		overlays: {
			"preaction.preserve": {
				status: "sent",
				emails: [e("2026-05-09", "out", "Formal Notice Follow Up: Resolution of Historical Dispossession and Fraud – Great House Farm, Llandough, Vale of Glamorgan", ME, "pensions@bp.com", "Notice on behalf of the Williams heirs to BP pensions / UK general enquiries.")]
			},
			"preaction.corporate": { status: "open" },
			"preaction.lbc": {
				status: "open",
				note: "Held until the current registered proprietor is named from official copies."
			}
		}
	},
	{
	...
	},
	{
		...
		policies: [{
			label: "Newbridge Real Estate",
			url: "https://www.newbridgerealestate.co.uk/"
		}],
		...
		overlays: {
			"enquiry.informal": {
				status: "sent",
				emails: [e("2026-08-21", "out", "Llandough Project, Cardiff", ME, "info@newbridgerealestate.co.uk", "Notice of a defect in land title; family claim of ownership; link to WhatDoTheyKnow 2020_01590_hyb.")]
			},
	...
	},
	{
		...
		policies: [
			{
				label: "SWP FOI",
				url: "https://www.south-wales.police.uk/rqo/request/foi/"
			},
			{
				label: "SWP subject access",
				url: "https://www.south-wales.police.uk/rqo/request/sar/"
			},
			{
				label: "Make a complaint",
				url: "https://www.south-wales.police.uk/advice/advice-and-information/complaints/"
			},
			{
				label: "IOPC",
		...
		overlays: {
			"foi.submit": {
				status: "sent",
				emails: [e("2026-08-20", "out", "Freedom of Information Request – Records relating to the deaths of William Buckler (1990) and Mary Doreen Williams (1983)", ME, "information@south-wales.police.uk", "FOIA request for recorded information relating to the two deaths."), e("2026-08-20", "in", "Automatic reply: Freedom of Information Request", "Freedom of Information <Information@south-wales.police.uk>", ME, "Auto-ack from the FOI team. Do not reply to the auto-ack.")]
			},
			"foi.ack": { status: "sent" },
			"sar.submit": {
				status: "open",
				note: "Separate SAR for personal data of named family members — do not mix with the deaths FOI."
			},
			"police.psd": {
				status: "open",
				note: "Schedule 3 complaint about 1988 enforcement and alleged removal of title documents is still remaining."
			}
		}
	},
	{
	...
	},
	{
		...
		policies: [
			{
				label: "CPS complaints",
				url: "https://www.cps.gov.uk/publication/complaints"
			},
			{
				label: "CPS FOI",
				url: "https://www.cps.gov.uk/publication-scheme"
			},
			{
				label: "Victim Liaison",
				url: "https://www.cps.gov.uk/victims-witnesses"
			}
		],
		...
		overlays: { "enquiry.informal": {
			status: "sent",
			emails: [e("2026-09-03", "out", "Re-Opening of BP vs Buckler 1987", ME, "VictimLiaison.Cymru-Wales@cps.gov.uk", "Formal placing of suspected historical fraud and related criminal conduct before the CPS."), e("2026-09-03", "in", "Automatic reply: External Email - Re-Opening of BP vs Buckler 1987", "Victim Liaison Cymru-Wales", ME, "Aim to acknowledge within 3 working days.")]
		} }
	},
	{
		id: "moj",
		...
		policies: [
			{
				label: "MoJ FOI",
				url: "https://www.gov.uk/government/organisations/ministry-of-justice/about/publication-scheme"
			},
			{
				label: "HMCTS data protection",
				url: "https://www.gov.uk/government/organisations/hm-courts-and-tribunals-service/about/personal-information-charter"
			},
			{
				label: "ICO SAR complaints",
				url: "https://ico.org.uk/make-a-complaint/data-protection-complaints-to-the-ico/"
			}
		],
		...
		overlays: {
			...
			"foi.review": {
				status: "open",
				note: "Available on the 'not held' refusal if the search statement is inadequate."
			},
			"sar.submit": {
				status: "exhausted",
				reference: "260803049 / 260702090"
			},
			"sar.response": {
				status: "exhausted",
				reference: "260803049",
			...
			"sar.review": {
				status: "review",
				reference: "260803049",
				emails: [e("2026-09-03", "out", "Re: Response to your request (our reference 260803049 - SAR)", ME, "NSUKilo@justice.gov.uk", "Internal review requested of the 3 September SAR response. 133 houses about to be developed.", "260803049")]
			}
		}
	},
	{
		...
		policies: [{
			label: "How to make an MOD FOI request",
			url: "https://www.gov.uk/government/publications/how-to-make-a-freedom-of-information-foi-request"
		}, {
			label: "ICO",
			url: "https://ico.org.uk/make-a-complaint/foi-and-eir-complaints/"
		}],
	...
	},
	{
		...
		overlays: {
			...
			"foi.response": {
				status: "replied",
				note: "WDTK 'new response' notices 2 Sep 2026. Open the WDTK thread and classify."
			},
			"foi.review": { status: "open" }
		}
	},
	{
		...
		policies: [{
			label: "NCA FOI",
			url: "https://www.nationalcrimeagency.gov.uk/who-we-are/freedom-of-information"
		}, {
			label: "Action Fraud (reporting)",
		...
		overlays: { "enquiry.informal": {
			status: "sent",
			note: "Referral mentioned in correspondence with Tim Knifton (NWSSP) on 24 Aug 2026. Confirm NCA/Action Fraud reference numbers."
		} }
	},
	{
		id: "action-fraud",
	...
	},
	{
		...
		policies: [{
			label: "Chief Coroner's guidance",
			url: "https://www.gov.uk/government/collections/chief-coroner-guidance-law-sheets-and-reports"
		}],
		...
		overlays: {
			...
			"coroner.archive": {
				status: "open",
				note: "Ask where pre-2002 Cardiff & Vale coroner files were stored and the destruction/transfer record."
			}
		}
	},
	{
		...
		policies: [
			{
				label: "FCA consumer contact",
				url: "https://www.fca.org.uk/consumers/contact-fca"
			},
			{
				label: "FCA FOI",
				url: "https://www.fca.org.uk/freedom-information"
			},
			{
				label: "FCA complaints (about the FCA)",
				url: "https://www.fca.org.uk/about/complaints-scheme"
			}
		],
		...
		overlays: {
			...
			"foi.submit": {
				status: "open",
				note: "Separate FOI: what the FCA holds on BP Pension Trust Ltd / BP Properties Ltd."
			}
		}
	},
	{
		...
		policies: [{
			label: "NHS Wales access to health records",
			url: "https://www.nhs.uk/using-the-nhs/about-the-nhs/how-to-access-your-health-records/"
		}, {
			label: "CAVUHB FOI",
			url: "https://cavuhb.nhs.wales/about-us/freedom-of-information/"
		}],
		...
		overlays: { "health.form": {
			status: "awaiting",
			emails: [e("2026-08-20", "out", "Access to Deceased Health Records / FOI Request – William Buckler (1990) and Mary Doreen Williams (1983)", ME, "CAV.AccessToRecords@wales.nhs.uk", "Request for health records of the two deceased."), e("2026-08-21", "in", "Re: Access to Deceased Health Records / FOI Request", "CAV.AccessToRecords@wales.nhs.uk", ME, "Application form and cover letter from Bernice Wheeler, Bereavement/Access to records supervisor.")],
	...
	},
	{
		...
		policies: [{
			label: "Medical Examiner Service",
			url: "https://www.gov.uk/government/publications/the-national-medical-examiner-system-in-england-and-wales"
		}],
	...
	},
	{
		...
		policies: [{
			label: "NWSSP FOI",
			url: "https://nwssp.nhs.wales/our-services/information-services/freedom-of-information/"
		}],
	...
	},
	{
		...
		policies: [{
			label: "Vale registration",
			url: "https://www.valeofglamorgan.gov.uk/en/living/registration/Registration-Service.aspx"
		}],
		notes: "Copied on the 20 Aug 2026 certificate request. Chase if Cardiff cannot produce Mary's entry.",
		overlays: { "enquiry.informal": { status: "sent" } }
	},
	{
		id: "doughty-mp",
		...
		policies: [{
			label: "PHSO — MP complaints about UK government",
			url: "https://www.ombudsman.org.uk/making-complaint"
		}, {
			label: "Parliamentary privilege / how to contact an MP",
			url: "https://www.parliament.uk/get-involved/contact-an-mp-or-lord/contact-your-mp/"
		}],
		...
		overlays: { "political.constituency": {
			status: "awaiting",
			emails: [
	...
	},
	{
		...
		policies: [{
			label: "Vale councillors",
			url: "https://www.valeofglamorgan.gov.uk/en/our_council/Councillors-and-Committees/Councillors/Councillors.aspx"
		}, {
			label: "PSOW (members' code)",
			url: "https://www.ombudsman.wales/how-to-complain/"
		}],
		...
		overlays: { "political.constituency": {
			status: "sent",
			emails: [e("2026-08-28", "out", "Great House Farm, Llandough", ME, "gdcarroll@valeofglamorgan.gov.uk", "Joint letter with Stephen Doughty MP.")]
		} }
	},
	{
		id: "cllr-franks",
	...
	},
	{
		...
		policies: [
			{
				label: "Wales Office",
				url: "https://www.gov.uk/government/organisations/office-of-the-secretary-of-state-for-wales"
			},
			{
				label: "FOI to UK government",
		...
		overlays: {
			"enquiry.informal": {
				status: "sent",
				emails: [e("2026-08-29", "out", "Williams Heirs, Great House Farm, Llandough (Est. 1667)", ME, "correspondence@ukgovwales.gov.uk", "Letter to the Secretary of State concerning Great House Farm / Ty Mawr.")]
			},
			"foi.submit": {
				status: "open",
				note: "Numbered FOI for Welsh Office 1988 listing/eviction papers still remaining."
			}
		}
	},
	{
		...
		policies: [{
			label: "Senedd committees",
			url: "https://senedd.wales/committees"
		}, {
			label: "Senedd petitions",
	...
	},
	{
		...
		overlays: { "foi.submit": {
			status: "open",
			note: "Do not assume ATISN 27021 searched MAG / Welsh Office inherited series."
		} }
	},
	{
		id: "ico",
		...
		policies: [
			...
			{
				label: "For the public — official information",
	...
	},
	{
		...
		overlays: { "regulator-own.ready": {
			status: "bounce",
			emails: [e("2026-05-09", "out", "Formal Notice Follow Up: Resolution of Historical Dispossession and Fraud – Great House Farm, Llandough, Vale of Glamorgan", ME, "ask@ombudsman.wales", "Formal notice BCC'd to the Ombudsman's ask inbox."), e("2026-05-09", "in", "RE: Formal Notice Follow Up: Resolution of Historical Dispossession and Fraud – Great House Farm, Llandough, Vale of Glamorgan", "ask@ombudsman.wales", ME, "We do not accept complaints into this inbox. Use the online form.")],
	...
	},
	{
		...
		policies: [
			{
				label: "Welsh Water contact",
				url: "https://www.dwrcymru.com/en/help-and-advice/contact-us"
			},
			{
				label: "EIR / information requests",
				url: "https://www.dwrcymru.com/en/about-us/governance/publication-scheme"
			},
			{
				label: "Consumer Council for Water",
				url: "https://www.ccwater.org.uk/make-a-complaint/"
			}
		],
		...
		overlays: {
			"enquiry.informal": {
				...
				emails: [e("2026-09-03", "in", "Welsh Water - Mail reference: 92994223/01", "CustomerCareTeam@dwrcymru.com", ME, "Further review of records relating to Great House Farm (Ty Mawr); searches across operating systems.", "92994223/01"), e("2026-09-03", "out", "Re: Welsh Water - Mail reference: 92994223/01", ME, "CustomerCareTeam@dwrcymru.com", "Thanks for the further searches.")]
			},
			"utility.gss": {
				status: "replied",
				reference: "92994223/01",
				emails: [e("2026-09-03", "out", "Re: Welsh Water - Mail reference: 92994223/01", ME, "CustomerCareTeam@dwrcymru.com", "Forwarding address for the £20 GSS payment: 5 Robin Hill, Dinas Powys CF64 4TF.")]
			},
			"foi.submit": {
				status: "open",
				note: "If the customer-care review did not cover historic mains/sewer maps, frame a numbered EIR."
			}
		}
	},
	{
		...
		emails: [
			"tryst.williams@reachplc.com",
			"wayne.davies@reachplc.com",
			"james.arnott@reachplc.com"
		],
		policies: [
			{
				label: "Reach plc privacy",
				url: "https://www.reachplc.com/legal/privacy"
			},
			{
				label: "IPSO",
				url: "https://www.ipso.co.uk/complain/"
			},
			{
				label: "Welsh Newspapers Online",
				url: "https://newspapers.library.wales/"
		...
		overlays: { "media.newsroom": {
			status: "awaiting",
			emails: [e("2026-08-20", "in", "Thanks for your email Re: Request for archive material / FOI-style enquiry – South Wales Echo coverage of William Buckler (1990)", "tryst.williams@reachplc.com", ME, "Out of office until Monday; Wayne Davies / James Arnott / newsdesk."), e("2026-09-02", "out", "Re: Thanks for your email Re: Request for archive material", ME, "tryst.williams@reachplc.com", "Follow-up on the archive request.")]
		} }
	},
	{
		id: "itv-wales",
		...
		policies: [{
			label: "ITV contact",
			url: "https://www.itv.com/contact"
		}, {
			label: "Ofcom (broadcast standards, not records)",
			url: "https://www.ofcom.org.uk/complaints"
		}],
		...
		overlays: { "media.newsroom": {
			status: "bounce",
			emails: [e("2026-07-21", "out", "Buckler vs United Kingdom", ME, "itvwales@itv.com", "BCC of the records/oversight letter."), e("2026-07-21", "in", "Delivery Status Notification (Failure)", "Mail Delivery Subsystem", ME, "Address not found: itvwales@itv.com.")],
	...
	},
	{
		...
		policies: [{
			label: "Bangor FOI",
			url: "https://www.bangor.ac.uk/governance-and-compliance/freedom-of-information"
		}],
		...
		overlays: { "enquiry.informal": {
			status: "sent",
			emails: [e("2026-08-30", "out", "Williams, Great House Farm, Llandough", ME, "g.j.williams@bangor.ac.uk", "Academic enquiry."), e("2026-08-30", "in", "Automatic reply: Williams, Great House Farm, Llandough", "g.j.williams@bangor.ac.uk", ME, "Annual leave, returning 1 Sep.")]
		} }
	},
	{
		id: "piddington",
		...
		policies: [{
			label: "Piddington PC",
			url: "https://www.piddingtonpc.org/"
		}],
		notes: "Email 24 Aug 2026 (Cedwyn Estate alias Llandough). Informal only.",
		overlays: { "enquiry.informal": {
			status: "sent",
			emails: [e("2026-08-24", "out", "Cedwyn Estate (alias Llandough, Cardiff)", ME, "piddington.parish.clerk@piddingtonpc.org", "Toponym / manorial parallel enquiry.")]
		} }
	},
	{
		id: "church-in-wales",
		...
		policies: [{
			label: "Church in Wales",
			url: "https://www.churchinwales.org.uk/"
		}, {
			label: "Glamorgan Archives parish collections",
			url: "https://glamarchives.gov.uk/"
		}],
	...
	},
	{
		...
		policies: [{
			label: "BBC Written Archives",
			url: "https://www.bbc.co.uk/archive/written-archives-centre"
		}, {
			label: "BBC FOI",
			url: "https://www.bbc.co.uk/foi/"
		}],
		...
		overlays: { "media.newsroom": {
			status: "awaiting",
			emails: [e("2026-05-09", "out", "Fwd: Formal Notice Follow Up: Resolution of Historical Dispossession and Fraud – Great House Farm, Llandough, Vale of Glamorgan", ME, "yourvoice@bbc.co.uk", "Forward of the formal notice."), e("2026-08-11", "in", "Thank you for emailing the BBC Written Archives Centre", "Heritage@bbc.co.uk", ME, "Auto-ack of a Written Archives request.")],
	...
	},
	{
		...
		policies: [{
			label: "Audit Wales",
			url: "https://www.audit.wales/"
		}, {
			label: "FOI",
			url: "https://www.audit.wales/about-us/freedom-information"
		}],
		...
		overlays: { "enquiry.informal": {
			status: "bounce",
			emails: [e("2026-07-21", "out", "Buckler vs United Kingdom", ME, "enquiries@audit.wales", "BCC of the records/oversight letter."), e("2026-07-21", "in", "Delivery Status Notification (Failure)", "Mail Delivery Subsystem", ME, "Address not found: enquiries@audit.wales.")],
	...
	},
	{
		...
		policies: [{
			label: "Landmark Chambers",
			url: "https://www.landmarkchambers.co.uk/"
		}],
	...
	},
	{
		...
		policies: [{
			label: "Falcon Chambers",
			url: "https://www.falcon-chambers.com/"
		}],
		notes: "Email 7 Aug 2026 seeking a preliminary view. No reply located. A polite chase, then another set if silent.",
		overlays: { "enquiry.informal": {
			status: "sent",
			emails: [e("2026-08-07", "out", "Proposed Restructuring of the Great House Farm (Ty Mawr) Forensic Report", ME, "clerks@falcon-chambers.com", "Seeking a preliminary view on a substantial body of historical land material.")]
		} }
	},
	{
		id: "senedd-members",
		...
		policies: [{
			label: "Find a Member",
			url: "https://senedd.wales/find-a-member-of-the-senedd/"
		}, {
			label: "Senedd petitions",
			url: "https://petitions.senedd.wales/"
		}],
		...
		overlays: {
			"political.constituency": {
				status: "bounce",
				emails: [e("2026-05-09", "out", "Formal Notice Follow Up: Resolution of Historical Dispossession and Fraud – Great House Farm, Llandough, Vale of Glamorgan", ME, "anthony.slaughter.ms@senedd.wales", "BCC of the formal notice."), e("2026-05-09", "in", "Delivery Status Notification (Failure)", "Mail Delivery Subsystem", ME, "anthony.slaughter.ms@, joseph.martin.ms@, huw.thomas.ms@, rhys.ab.owen@ not found.")],
...
function isInFlight(s) {
	return s === "sent" || s === "awaiting" || s === "replied" || s === "review" || s === "complaint";
}
var SECTOR_LABEL = {
	heritage: "heritage",
	archive: "archives",
...
function avenueNode(a, partyId, parentPath, sector) {
	...
	const rolled = children.length ? rollup(children) : {
		remaining,
		depleted,
		status: a.status
	};
...
function sectorNode(sector, parties) {
...
function buildCaseTree() {
	...
	const sectorNodes = SECTOR_ORDER.filter((s) => bySector.has(s)).map((s) => sectorNode(s, bySector.get(s) ?? []));
...
function listRemaining(filter) {
	const tree = buildCaseTree();
	const out = [];
	const walk = (n) => {
		if (!n.children.length && !isDepleted(n.status) && n.id !== "root" && !n.path.startsWith("sector:") && (!filter?.partyId || n.partyId === filter.partyId) && (!filter?.sector || n.sector === filter.sector)) out.push(n);
		n.children.forEach(walk);
	};
...
function searchTree(query) {
	...
	const walk = (n) => {
		...
		].filter(Boolean).join(" ").toLowerCase().includes(q) && n.id !== "root" && !n.path.startsWith("sector:")) out.push(n);
		n.children.forEach(walk);
	};
	walk(tree.root);
	return out;
...
var Route$4 = createFileRoute("/llms.txt")({ server: { handlers: { GET: async () => {
	const tree = buildCaseTree();
...
var Route$3 = createFileRoute("/api/remaining.json")({ server: { handlers: { GET: async ({ request }) => {
	const url = new URL(request.url);
	const rows = listRemaining({
		partyId: url.searchParams.get("partyId") ?? void 0,
		sector: url.searchParams.get("sector") ?? void 0
	}).map((n) => ({
		path: n.path,
...
var Route$2 = createFileRoute("/api/summary.json")({ server: { handlers: { GET: async () => Response.json(exhaustionSummary(), { headers: { "cache-control": "public, max-age=60" } }) } } });
var Route$1 = createFileRoute("/api/tree.json")({ server: { handlers: { GET: async () => {
	const tree = buildCaseTree();
	return Response.json({
		name: "Enquiry",
		snapshotDate: tree.snapshotDate,
...
var Route = createFileRoute("/api/parties/$id")({ server: { handlers: { GET: async ({ params }) => {
...
var rootRouteChildren = {
	...
	ApiRemainingDotjsonRoute: Route$3.update({
		id: "/api/remaining.json",
		path: "/api/remaining.json",
		getParentRoute: () => Route$6
	}),
	ApiSummaryDotjsonRoute: Route$2.update({
		id: "/api/summary.json",
		path: "/api/summary.json",
		getParentRoute: () => Route$6
	}),
	ApiTreeDotjsonRoute: Route$1.update({
		id: "/api/tree.json",
		path: "/api/tree.json",
		getParentRoute: () => Route$6
	}),
	ApiPartiesIdRoute: Route.update({
		id: "/api/parties/$id",
		path: "/api/parties/$id",
		getParentRoute: () => Route$6
	})
...
//#endregion
export { listRemaining as a, STATUS_LABEL as c, findNode as i, isDepleted as l, buildCaseTree as n, partyById as o, exhaustionSummary as r, searchTree as s, router_exports as t, isInFlight as u };
